    ...
    if ($con = connect()){
        ...
        if ($uc = mysqli_query($con, $usercall)){
            if (0 < mysqli_num_rows($uc)){
                ...
                $nsql = "SELECT f.id, u.name, u.dp, u.username, u.type FROM follow f INNER JOIN users u ON f.following=u.mail WHERE follower LIKE '{$email_caught}' AND  request NOT LIKE 'true' ORDER BY f.id DESC LIMIT {$start}, 20";
